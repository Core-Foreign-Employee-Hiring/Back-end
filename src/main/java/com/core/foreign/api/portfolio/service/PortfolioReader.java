...
public class PortfolioReader {
    ...
    public BasicPortfolioDTO getBasicPortfolio(Employee employee) {
        ...
        EmployeePortfolio employeePortfolio = employeePortfolioRepository.findByEmployeeId(employee.getId(), EmployeePortfolioStatus.COMPLETED)
                .orElseGet(() -> {
                    log.error("완성된 포트 폴리오가 없음. employeeId= {}", employee.getId());
                    return null;
                });
